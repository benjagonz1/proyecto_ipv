-- Corregido: db_ipv_proyecto (fix)
-- Fecha de generación: 19-11-2025
-- Nota: Se corrigió una FK invertida en el volcado original.
...
--
-- Nota: en el volcado original esta restricción referenciaba por error a `inspecciones(id_motivo)`.
-- Se omite esa FK errónea y en su lugar se crea la FK correcta desde `inspecciones.id_motivo` hacia `motivos_inspeccion.id_motivo`.
--
-- Añadimos la restricción correcta: `inspecciones.id_motivo` -> `motivos_inspeccion.id_motivo`
ALTER TABLE `inspecciones`
  ADD CONSTRAINT `inspecciones_motivo_ibfk_1` FOREIGN KEY (`id_motivo`) REFERENCES `motivos_inspeccion` (`id_motivo`) ON DELETE RESTRICT ON UPDATE CASCADE;
